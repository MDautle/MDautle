...
FC_neg_cols_mouse3_GOI <- grep("^FC_neg.*mouse3$", colnames(memory_related_genes_expression))
FC_neg_mouse3_GOI <- select(memory_related_genes_expression,all_of(FC_neg_cols_mouse3_GOI))
FC_neg_mouse3_GOI_weight.matrix <- GENIE3(FC_neg_mouse3_GOI)
FC_neg_mouse3_GOI_linked.list <- get.link.list(FC_neg_mouse3_GOI_weight.matrix)
write.csv(FC_neg_mouse3_GOI_linked.list, file = "Remote_memory_related_genes_results/FC_neg_mouse3_GENIE3_linkedlist_MemoryRelatedOnly", row.names=F)

g_prep <- graph.data.frame(FC_neg_mouse3_GOI_linked.list, directed = F)
g <- get.adjacency(g_prep)
g_post <- graph.adjacency(g, mode = 'undirected',weighted= T)
g.cyto <-igraph.to.graphNEL(g_post)

network_g <- createNetworkFromGraph("FC_neg_mouse3", graph=g.cyto)
