...
FC_pos_cols_mouse2_GOI <- grep("^FC_pos.*mouse2$", colnames(memory_related_genes_expression))
FC_pos_mouse2_GOI <- select(memory_related_genes_expression,all_of(FC_pos_cols_mouse2_GOI))
FC_pos_mouse2_GOI_weight.matrix <- GENIE3(FC_pos_mouse2_GOI)
FC_pos_mouse2_GOI_linked.list <- get.link.list(FC_pos_mouse2_GOI_weight.matrix, report.max = 200)
write.csv(FC_pos_mouse2_GOI_linked.list, file = "Remote_memory_related_genes_results/FC_pos_mouse2_GENIE3_linkedlist_MemoryRelatedOnly_Top200.csv", row.names=F)

#Create GRN in Cytoscape
g_prep <- graph.data.frame(FC_pos_mouse2_GOI_linked.list, directed = F)
...
network_g <- createNetworkFromGraph("FC_pos_mouse2", graph=g.cyto)
